        ...
        <div className="p-6 md:p-8 space-y-6">
            ...
            <div className="flex flex-wrap gap-3 items-center">
                <div className="flex gap-1 bg-slate-800/40 rounded-lg p-1">
                    <button onClick={() => setViewMode("kanban")} className={`px-3 py-1.5 rounded-md text-xs font-medium ${viewMode === "kanban" ? "bg-blue-600/20 text-blue-400" : "text-slate-400"}`}>Kanban</button>
                    <button onClick={() => setViewMode("list")} className={`px-3 py-1.5 rounded-md text-xs font-medium ${viewMode === "list" ? "bg-blue-600/20 text-blue-400" : "text-slate-400"}`}>Lista</button>
                </div>
            ...
            {!loading && viewMode === "kanban" ? (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                    {columns.map(col => {
                        ...
                        return (
                            <motion.div key={col.status} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                                className={`bg-slate-800/30 rounded-xl border border-white/[0.06] border-t-2 ${col.color} overflow-hidden`}>
                                <div className="px-4 py-3 border-b border-slate-700/30 flex items-center justify-between">
                                    <h3 className="text-xs font-bold text-white uppercase tracking-widest">{col.label}</h3>
                                    <span className="px-2 py-0.5 text-[10px] font-bold bg-slate-700/50 rounded-full text-slate-400">{colTasks.length}</span>
                                </div>
                                <div className="p-3 space-y-2 min-h-[200px]">
                                    {colTasks.map(task => {
                                        const name = task.assignee?.name || "Não atribuído";
                                        return (
                                            <div key={task.id} onClick={() => setSelectedTask(apiTaskToMock(task))}
                                                className="p-3 rounded-lg bg-slate-900/60 border border-slate-700/30 hover:border-blue-600/30 transition-all cursor-pointer group">
                                                <div className="flex items-start gap-2">
                                                    <GripVertical size={14} className="text-slate-600 shrink-0 mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity" />
                                                    <div className="flex-1 min-w-0">
                                                        <p className="text-sm text-white font-medium truncate">{task.title}</p>
                                                        <div className="flex items-center gap-2 mt-2">
                                                            <span className={`px-1.5 py-0.5 text-[9px] font-bold rounded ${getPriorityColor(task.priority)}`}>{getPriorityLabel(task.priority)}</span>
                                                            {task.blocked && <span className="px-1.5 py-0.5 text-[9px] font-bold rounded bg-red-500/10 text-red-400 animate-pulse">Bloqueada</span>}
                                                        </div>
                                                        <div className="flex items-center gap-2 mt-2">
                                                            <div className="w-5 h-5 rounded-full bg-gradient-to-tr from-blue-600 to-blue-400 flex items-center justify-center text-white text-[8px] font-bold">{name.charAt(0)}</div>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                    className="bg-slate-800/40 rounded-xl border border-white/[0.06] overflow-hidden">
                    ...
                    <table className="w-full text-sm min-w-[500px]">
                        ...
                        <tbody>
                            {tasks.map(task => {
                                const name = task.assignee?.name || "Não atribuído";
                                return (
                                    <tr key={task.id} onClick={() => setSelectedTask(apiTaskToMock(task))} className="border-b border-slate-700/20 hover:bg-slate-800/30 transition-colors cursor-pointer">
                                        <td className="px-5 py-3 text-white font-medium">{task.title}{task.blocked && <span className="ml-2 text-[9px] text-red-400 font-bold">BLOQUEADA</span>}</td>
                                        <td className="px-5 py-3"><span className="px-2 py-0.5 text-[11px] font-medium bg-slate-700/50 rounded-md text-slate-300">{getStatusLabel(task.status)}</span></td>
                                        <td className="px-5 py-3"><span className={`px-2 py-0.5 text-[10px] font-bold rounded-md ${getPriorityColor(task.priority)}`}>{getPriorityLabel(task.priority)}</span></td>
                                        <td className="px-5 py-3 text-slate-400 text-xs">{name}</td>
                                    </tr>
                                );
                            })}
                        </tbody>

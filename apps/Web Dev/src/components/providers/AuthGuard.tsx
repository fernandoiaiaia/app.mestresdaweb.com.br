...
export function AuthGuard({ children }: { children: React.ReactNode }) {
    ...
    // App-level access check: Dev app requires "dev" in allowedApps
    // OWNER and ADMIN bypass this check
    const hasAppAccess =
        !user ||
        user.role === "OWNER" ||
        user.role === "ADMIN" ||
        (user.allowedApps && user.allowedApps.includes("dev"));
            ...
            <div className="min-h-screen bg-slate-900 flex items-center justify-center">
                <div className="flex flex-col items-center gap-4 max-w-md text-center px-6">
                    ...
                    <p className="text-sm text-slate-400 leading-relaxed">
                        Sua conta não tem permissão para acessar o <span className="text-blue-400 font-semibold">Painel de Desenvolvimento</span>. 

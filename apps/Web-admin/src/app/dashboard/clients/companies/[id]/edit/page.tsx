...
export default function EditCompanyPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        Promise.all([
            ...
            if (compRes?.success && compRes.data) {
                ...
                // Pre-populate linked contacts
                if (c.clients && c.clients.length > 0) {
                    setLinkedContacts(c.clients.map((cl: any) => ({
                        id: cl.id,
                        name: cl.name,
                        email: cl.email,
                        phone: cl.phone,
    ...
    const unlinkContact = (cid: string) => {
        setLinkedContacts(prev => prev.filter(c => c.id !== cid));
    };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (res.success) {
                setSaved(true);
                setTimeout(() => { router.push("/dashboard/clients/companies"); }, 1500);
            } else {
        ...
        <div className="p-6 md:p-10 max-w-3xl mx-auto">
            ...
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
                <div className="flex items-center gap-2 mb-4">
                    <Link href="/dashboard/clients/companies" className="flex items-center gap-1.5 text-slate-500 hover:text-slate-300 transition-colors text-sm">
                        <ChevronLeft size={16} /><Building2 size={14} /><span>Empresas</span>
                    </Link>
                    <span className="text-slate-700">/</span>
                    <span className="text-slate-300 text-sm font-medium">Editar Empresa</span>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-400">Atualize os dados da empresa</p>
                    </div>
                </div>
            </motion.div>
            ...
            <div className="space-y-6">
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.13 }} className="bg-slate-800/40 border border-white/[0.06] rounded-2xl p-6">
                    ...
                    <p className="text-[10px] text-slate-500 mb-4">Busque e selecione contatos para atrelar a esta empresa</p>
            ...
            <div className="flex justify-between items-center mt-8 pt-6 border-t border-white/[0.04]">
                <Link href="/dashboard/clients/companies" className="px-6 py-2.5 rounded-xl font-medium text-slate-400 hover:text-white hover:bg-slate-800 transition-colors text-sm">
                    Cancelar
                </Link>

...
const ROLE_SUGGESTIONS = [
    "Diretor de Marketing", "Diretor Financeiro", "Diretor Comercial", "Diretor de TI",
...
function RoleCombobox({ value, onChange }: { value: string; onChange: (v: string) => void }) {
    ...
    const filtered = ROLE_SUGGESTIONS.filter(r => r.toLowerCase().includes(value.toLowerCase()));
    return (
        <div>
            ...
            <div className="relative" ref={ref}>
                ...
                <input type="text" value={value} onChange={e => { onChange(e.target.value); setOpen(true); }} onFocus={() => setOpen(true)} placeholder="Buscar ou selecionar cargo..." className="w-full pl-10 pr-4 py-2.5 bg-slate-800/50 border border-white/[0.08] rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500/40 transition-colors" />
                {open && (
                    <div className="absolute z-50 mt-1 w-full max-h-48 overflow-y-auto bg-slate-900 border border-white/10 rounded-xl shadow-xl">
                        {filtered.length > 0 ? filtered.map(role => (
                            <button key={role} type="button" onClick={() => { onChange(role); setOpen(false); }} className={`w-full text-left px-4 py-2 text-sm transition-colors hover:bg-blue-500/10 ${value === role ? 'bg-blue-500/10 text-blue-400 font-bold' : 'text-slate-300'}`}>{role}</button>
                        )) : <div className="px-4 py-3 text-xs text-slate-500 text-center">Nenhum cargo encontrado</div>}
                    </div>
                )}
            </div>
        </div>
    ...
    return (
        <div className={span === 2 ? "md:col-span-2" : ""}>
            ...
            <div className="relative">
                ...
                <input type={type} value={value} onChange={e => onChange(field, e.target.value)} placeholder={placeholder} className={`w-full pl-10 pr-4 py-2.5 bg-slate-800/50 border rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none transition-colors ${error ? 'border-red-500/50 focus:border-red-500' : 'border-white/[0.08] focus:border-blue-500/40'}`} />
            </div>
...
interface CompanyOption { id: string; name: string; cnpj: string | null; segment: string | null; }
...
export default function EditContactPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        ...
        ]).then(([clientRes, compRes, segRes]) => {
            if (clientRes?.success && clientRes.data) {
                const c = clientRes.data;
                // Parse tags/social from notes
                let notes = c.notes || "";
                let instagram = "", linkedin = "";
                const tagsArr: string[] = [];
                if (notes.includes("---")) {
                    const [mainNotes, extras] = notes.split("---");
                    notes = mainNotes.trim();
                    extras.split("\n").forEach((line: string) => {
                        const l = line.trim();
                        if (l.startsWith("LinkedIn:")) linkedin = l.replace("LinkedIn:", "").trim();
                        else if (l.startsWith("Instagram:")) instagram = l.replace("Instagram:", "").trim();
                        else if (l.startsWith("Tags:")) tagsArr.push(...l.replace("Tags:", "").trim().split(",").map((t: string) => t.trim()).filter(Boolean));
                    });
                }
                setForm({
                    nome: c.name || "",
                ...
                if (c.companyId && c.companyRef) {
                    setSelectedCompany({ id: c.companyRef.id, name: c.companyRef.name, cnpj: null, segment: null });
                } else if (c.companyId) {
                    setSelectedCompany({ id: c.companyId, name: c.company || "Empresa", cnpj: null, segment: null });
                }
            }
    ...
    useEffect(() => {
        const h = (e: MouseEvent) => { if (companyRef.current && !companyRef.current.contains(e.target as Node)) setShowCompanyDropdown(false); };
        document.addEventListener("mousedown", h);
        return () => document.removeEventListener("mousedown", h);
    }, []);
    ...
    const update = (key: string, value: string) => {
        let fv = value;
        if (key === "celular") fv = maskPhone(value);
        setForm(prev => ({ ...prev, [key]: fv }));
    ...
    const handleSave = async () => {
        ...
        try {
            const res = await api(`/api/clients/${id}`, { method: "PUT", body: payload });
            if (res.success) {
                setSaved(true);
                setTimeout(() => router.push("/dashboard/clients/contacts"), 1500);
            } else {
                toast.error("Erro ao salvar", res.message || "Tente novamente.");
            }
        } catch (error) {
            console.error(error);
    ...
    if (loading) {
        return (
            <div className="p-6 md:p-10 max-w-3xl mx-auto flex items-center justify-center min-h-[50vh]">
        ...
        <div className="p-6 md:p-10 max-w-3xl mx-auto">
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
                <div className="flex items-center gap-2 mb-4">
                    <Link href="/dashboard/clients/contacts" className="flex items-center gap-1.5 text-slate-500 hover:text-slate-300 transition-colors text-sm">
                        <ChevronLeft size={16} /><User size={14} /><span>Contatos</span>
                    </Link>
                    <span className="text-slate-700">/</span>
                    <span className="text-slate-300 text-sm font-medium">Editar Contato</span>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-white tracking-tight">Editar Contato</h1>
                        <p className="text-sm text-slate-400">Atualize os dados do contato</p>
                    </div>
                </div>
            </motion.div>
            ...
            <div className="space-y-6">
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }} className="bg-slate-800/40 border border-white/[0.06] rounded-2xl p-6">
                    <div className="flex items-center justify-between mb-4">
                        <div>
                            ...
                            <p className="text-[10px] text-slate-500 mt-1">Defina ou altere a senha de acesso ao Web Cliente deste contato.</p>
                        </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                        ...
                        <InputField label="Nova Senha (Opcional)" field="portalPassword" type="text" value={form.portalPassword} placeholder="Deixe em branco para manter..." icon={Lock} onChange={update} />
                    </div>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="bg-slate-800/40 border border-white/[0.06] rounded-2xl p-6">
                    ...
                    {selectedCompany ? (
                        <div className="flex items-center justify-between p-3 bg-slate-900/50 border border-blue-500/20 rounded-xl">
                            <div className="flex items-center gap-3">
                                <div className="w-9 h-9 rounded-lg bg-gradient-to-tr from-blue-600 to-cyan-400 flex items-center justify-center text-white text-[10px] font-bold shrink-0">{selectedCompany.name.substring(0, 2).toUpperCase()}</div>
                                <span className="text-sm font-medium text-white">{selectedCompany.name}</span>
                            </div>
                            <button onClick={() => setSelectedCompany(null)} className="p-1.5 text-slate-500 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-all"><X size={14} /></button>
                        </div>
                    ) : (
                        <div ref={companyRef} className="relative">
                            <Search size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500 pointer-events-none" />
                            <input type="text" value={companySearch} onChange={e => { setCompanySearch(e.target.value); setShowCompanyDropdown(true); }} onFocus={() => setShowCompanyDropdown(true)} placeholder="Buscar empresa..." className="w-full pl-10 pr-4 py-2.5 bg-slate-800/50 border border-white/[0.08] rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500/40 transition-colors" />
                            {showCompanyDropdown && (
                                <div className="absolute z-50 mt-1 w-full max-h-56 overflow-y-auto bg-slate-900 border border-white/10 rounded-xl shadow-xl">
                                    {filteredCompanies.length > 0 ? filteredCompanies.slice(0, 15).map(c => (
                                        <button key={c.id} type="button" onClick={() => { setSelectedCompany(c); setCompanySearch(""); setShowCompanyDropdown(false); }} className="w-full text-left px-4 py-2.5 hover:bg-blue-500/10 transition-colors flex items-center gap-3 border-b border-white/[0.03] last:border-0">
                                            <div className="w-7 h-7 rounded-lg bg-gradient-to-tr from-blue-600 to-cyan-400 flex items-center justify-center text-white text-[8px] font-bold shrink-0">{c.name.substring(0, 2).toUpperCase()}</div>
                                            <span className="text-sm text-white font-medium truncate">{c.name}</span>
                                        </button>
                                    )) : <div className="px-4 py-4 text-xs text-slate-500 text-center">Nenhuma empresa encontrada</div>}
                                </div>
                            )}
                        </div>
                    )}
                        ...
                        <div className="flex flex-wrap gap-1.5">
                            {form.tags.map(tag => (
                                <span key={tag} className="flex items-center gap-1 px-2.5 py-1 bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 rounded-lg text-[11px] font-medium">
                                    {tag}<button onClick={() => removeTag(tag)} className="ml-0.5 hover:text-red-400 transition-colors">×</button>
                                </span>
                            ))}
                        </div>
                    )}
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-slate-800/40 border border-white/[0.06] rounded-2xl p-6">
                    ...
                    <textarea value={form.observacoes} onChange={e => update("observacoes", e.target.value)} rows={4} placeholder="Informações relevantes..." className="w-full px-4 py-3 bg-slate-800/50 border border-white/[0.08] rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500/40 resize-none" />
                </motion.div>
            </div>

            {/* Bottom Actions */}
            <div className="flex justify-between items-center mt-8 pt-6 border-t border-white/[0.04]">
                <Link href="/dashboard/clients/contacts" className="px-6 py-2.5 rounded-xl font-medium text-slate-400 hover:text-white hover:bg-slate-800 transition-colors text-sm">Cancelar</Link>

        ...
        try {
            ...
            if (fn.endsWith(".csv")) {
                data = await new Promise<any[][]>((resolve, reject) => {
                    Papa.parse(file, { header: false, skipEmptyLines: true, complete: r => resolve(r.data as any[][]), error: reject });
                });
            ...
            if (!data || data.length < 2) { toast.warning("Nenhum dado encontrado."); return; }

            // Remove a primeira linha (cabeçalhos)
            let rawRows = data.slice(1);
            
            // Fix bad CSV parsing and filter empty rows
            let rows = rawRows.map(r => {
                if (r && r.length === 1 && typeof r[0] === 'string' && r[0].includes(';')) {
                    return r[0].split(';');
                }
                return r;
            }).filter(r => r && r.length > 2 && r.some(c => !!String(c).trim()));

            if (!rows.length) { toast.warning("A planilha parece estar vazia ou mal formatada."); return; }

            // ─── Parse value from Brazilian format (R$ 1.234,56) ─────────────────
            ...
            // ─── Parse date (DD/MM/YYYY or other formats) ────────────────────────
            const parseDate = (raw: any): string | undefined => {
                if (!raw) return undefined;
                const str = String(raw).trim();
                
                // Try Excel date serial number (e.g. 45000 is around 2023)
                if (/^\d{4,5}$/.test(str)) {
                ...
                // Try ISO or other native parseable formats
                const d = new Date(str);
                // Ensure the date is reasonable (between year 2000 and 2100) to avoid +045000 years
            ...
            // ─── Map each row to the import payload ──────────────────────────────
            const items = rows.map(row => {
                // A: Etapa
                const stageName = row[0] ? String(row[0]).trim() : null;
                // B: Origem da oportunidade
                const dealSource = row[1] ? String(row[1]).trim() : "Importação";
                // C: Data
                const createdAtRaw = row[2];
                const createdAt = parseDate(createdAtRaw);
                // D: Nome da oportunidade
                const dealTitle = row[3] ? String(row[3]).trim() : null;
                // E: Valor
                const dealValue = parseValue(row[4]);
                // F: Campanha
                const campaignNote = row[5] ? String(row[5]).trim() : null;
                // G: Nome do contato
                const name = row[6] ? String(row[6]).trim() : "Contato Importado";
                // H: E-mail
                const email = row[7] ? String(row[7]).trim() : null;
                // I: Responsável
                const consultantName = row[8] ? String(row[8]).trim() : null;
                ...
                return {
                    ...
                    stageName,
                    consultantName,
                    dealTitle: dealTitle || name,
                    dealValue,
                    dealTags: [],
                    dealSource,
                    expectedClose: null,
                    dealOrigin: dealSource,
                    campaignNote
                };
            });

            const res = await api<{ clientsCreated: number; dealsCreated: number; errors: string[] }>(
                "/api/clients/bulk-with-deals",
                { method: "POST", body: { items } }
            );
            if (res.success && res.data) {
                toast.success(`Importação concluída! ${res.data.clientsCreated} contatos e ${res.data.dealsCreated} oportunidades criados.`);
                if (res.data.errors && res.data.errors.length > 0) {
                    console.warn("Erros na importação:", res.data.errors);
                    toast.warning(`${res.data.errors.length} linhas com erro. Verifique o console.`);
                }
                loadClients();
            } else {
                toast.error("Erro na importação: " + (res.message || "Erro desconhecido"));
            }
            ...
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
                <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
                    <div className="flex items-center gap-3">
                        <Link href="/dashboard/clients" className="flex items-center gap-1 text-slate-500 hover:text-slate-300 transition-colors text-sm mr-1">
                            <ChevronLeft size={16} />
                        </Link>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <Link href="/dashboard/clients/contacts/new" className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-semibold text-sm transition-all shadow-lg shadow-blue-600/20">
                            ...
                            >
                                ...
                                <td className="px-5 py-3">
                                    <div className="flex items-center gap-1">
                                        <Link href={`/dashboard/clients/contacts/${c.id}/edit`} className="p-1.5 rounded-lg text-slate-600 hover:text-blue-400 hover:bg-blue-400/10 transition-all opacity-0 group-hover:opacity-100"><Edit3 size={14} /></Link>

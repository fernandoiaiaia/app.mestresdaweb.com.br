...
export default function PaymentMethodsPage() {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            toast.error("Erro ao carregar dados", "Não foi possível carregar as formas de pagamento.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            if (editingItem) {
                const res = await paymentMethodsService.update(editingItem.id, formData);
                if (res.success) {
                    toast.success("Sucesso", "Forma de pagamento atualizada com sucesso!");
                    loadData();
                    handleCloseModal();
                } else {
                ...
                const res = await paymentMethodsService.create(formData);
                if (res.success) {
                    toast.success("Sucesso", "Forma de pagamento criada com sucesso!");
                    loadData();
                    handleCloseModal();
                } else {
    ...
    const handleDelete = async (id: string, name: string) => {
        ...
        try {
            const res = await paymentMethodsService.delete(id);
            if (res.success) {
                toast.success("Sucesso", "Excluída com sucesso.");
                loadData();
            } else {
    ...
    const toggleActiveStatus = async (item: PaymentMethod) => {
        try {
            const res = await paymentMethodsService.update(item.id, { active: !item.active });
            if (res.success) {
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 mb-8">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-400">Configure os métodos de pagamento (PIX, Cartão, Boleto, etc)</p>
                    </div>
                </div>
                <button 
                                ...
                                <div className="px-6 py-4 border-b border-white/[0.06] flex items-center justify-between bg-slate-800/40">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-400">Preencha o nome do método</p>
                                        </div>
                                    </div>

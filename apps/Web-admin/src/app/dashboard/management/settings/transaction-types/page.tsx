...
export default function TransactionTypesPage() {
    ...
    const loadData = async () => {
        setIsLoading(true);
        try {
            const res = await transactionTypesService.list();
            if (res.success && res.data) {
                setItems(res.data);
            }
        } catch (e) {
            toast.error("Erro ao carregar dados", "Não foi possível carregar os tipos.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const filtered = items.filter(c => {
        ...
        return c.name.toLowerCase().includes(q) || c.nature.toLowerCase().includes(q);
    });

    const handleOpenCreate = () => {
        setEditingItem(null);
        setFormData({ name: "", nature: "income", active: true });
        setIsModalOpen(true);
    };

    const handleOpenEdit = (item: TransactionType) => {
        setEditingItem(item);
        setFormData({
            name: item.name,
            nature: item.nature,
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        if (!formData.name) {
            toast.error("Atenção", "O nome é obrigatório.");
            return;
        }

        try {
            if (editingItem) {
                const res = await transactionTypesService.update(editingItem.id, formData);
                if (res.success) {
                    toast.success("Sucesso", "Tipo atualizado com sucesso!");
                    loadData();
                    handleCloseModal();
                } else {
                ...
                const res = await transactionTypesService.create(formData);
                if (res.success) {
                    toast.success("Sucesso", "Tipo criado com sucesso!");
                    loadData();
                    handleCloseModal();
                } else {
    ...
    const handleDelete = async (id: string, name: string) => {
        const confirmed = await confirm({
            ...
            description: `Deseja realmente excluir "${name}"? Esta ação não pode ser desfeita.`,
        ...
        try {
            const res = await transactionTypesService.delete(id);
            if (res.success) {
                toast.success("Sucesso", "Excluído com sucesso.");
                loadData();
            } else {
    ...
    const toggleActiveStatus = async (item: TransactionType) => {
        try {
            const res = await transactionTypesService.update(item.id, { active: !item.active });
            if (res.success) {
                toast.success("Sucesso", `Status alterado.`);
                loadData();
            }
        } catch (e) {
        ...
        <div className="p-6 md:p-10 max-w-7xl mx-auto animate-in fade-in duration-500">
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 mb-8">
                <div className="flex items-center gap-3">
                    <Link href="/dashboard/management/settings/financial" className="flex items-center gap-1 text-slate-500 hover:text-slate-300 transition-colors text-sm mr-1">
                        <ChevronLeft size={16} />
                    </Link>
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-white tracking-tight">Tipos de Lançamento</h1>
                        <p className="text-sm text-slate-400">Gerencie os grupos financeiros de receitas e despesas</p>
                    </div>
                </div>
                <button 
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="text-xs text-slate-400 bg-slate-900/40 uppercase border-b border-white/[0.04]">
                            <tr>
                                ...
                                <th className="px-5 py-4 font-medium tracking-wider">Natureza</th>
                            ...
                            {isLoading ? (
                                ...
                                filtered.map(item => (
                                    <tr key={item.id} className="hover:bg-slate-800/60 transition-colors group">
                                        ...
                                        <td className="px-5 py-4">
                                            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${item.nature === 'income' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border-rose-500/20'}`}>
                                                {item.nature === 'income' ? 'Receita' : 'Despesa'}
                                            </span>
                                        </td>
                        ...
                        >
                            <form onSubmit={handleSave}>
                                ...
                                <div className="p-6 space-y-4">
                                    <div>
                                        <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Natureza *</label>
                                        <select
                                            required
                                    ...
                                    <div>
                                        <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Nome do Tipo *</label>
                                        <input 
                                            type="text" 
                                            required
                                            placeholder="Ex: Recebimentos, Despesas Fixas..."
                                            value={formData.name}
                                    ...
                                    <div className="flex items-center gap-3 pt-2">
                                        <label className="relative inline-flex items-center cursor-pointer">
                                            ...
                                            <span className="ml-3 text-sm font-medium text-slate-300">Ativo</span>
                                        </label>
                                    </div>
                                </div>

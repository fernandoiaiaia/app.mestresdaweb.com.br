...
export default function CorporateUsersPage() {
    ...
    const loadUsers = async () => {
        ...
        try {
            ...
            if (res && res.success && res.data) {
                // Filter only users that have access to "corporate" app
                const corporateUsers = res.data.filter((u) => {
                    if (!u.allowedApps || u.allowedApps.length === 0) return true; // Show users without app restrictions
                    return u.allowedApps.includes("corporate");
                });
                const mapped: UserView[] = corporateUsers.map((u) => ({
                    id: u.id,
                    name: u.name,
                    email: u.email,
                    phone: u.phone || "",
    ...
    return (
        <div className="p-6 md:p-10 max-w-7xl mx-auto">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-400">Cadastre e gerencie acessos ao painel Corporate</p>
                        </div>
                    </div>
                    <Link
                        href="/dashboard/management/settings/users/new"
                        className="flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-semibold text-sm transition-all duration-300 shadow-lg shadow-emerald-600/20 hover:shadow-emerald-500/30"
                    >
                        <Plus size={18} />
                        Novo Usuário
                    </Link>
            ...
            >
                {[
                    { label: "Total", value: counts.total, color: "text-white", bg: "bg-slate-700/30", border: "border-white/[0.06]" },
                    { label: "Ativos", value: counts.ativo, color: "text-emerald-400", bg: "bg-emerald-500/5", border: "border-emerald-500/10" },
                    { label: "Inativos", value: counts.inativo, color: "text-slate-400", bg: "bg-slate-500/5", border: "border-slate-500/10" },
                    { label: "Pendentes", value: counts.pendente, color: "text-amber-400", bg: "bg-amber-500/5", border: "border-amber-500/10" },
                ].map((stat) => (
                    <div key={stat.label} className={`${stat.bg} backdrop-blur-sm border ${stat.border} rounded-xl p-4`}>
                ...
                <div className="flex flex-col md:flex-row gap-3">
                    ...
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            className="w-full pl-11 pr-4 py-2.5 bg-slate-800/50 border border-white/[0.08] rounded-xl text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-emerald-500/40 focus:ring-1 focus:ring-emerald-500/20 transition-all"
                        />
                    </div>
                    {/* Filter toggle */}
                    <button
                        onClick={() => setShowFilters(!showFilters)}
                        className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all border ${showFilters ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-slate-800/50 border-white/[0.08] text-slate-400 hover:text-white'}`}
                    >
                        <Filter size={16} />
                        Filtros
                            ...
                            <div className="flex flex-wrap gap-3 mt-3 p-4 bg-slate-800/30 border border-white/[0.04] rounded-xl">
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="bg-slate-800 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-emerald-500/40 min-w-[150px]"
                                    >
                    ...
                    <div className="divide-y divide-white/[0.04]">
                        {filteredUsers.map((user, index) => (
                            ...
                            >
                                {/* User Info */}
                                <div className="flex items-center gap-3">
                                    <div className={`w-10 h-10 rounded-full bg-gradient-to-tr ${avatarGradients[index % avatarGradients.length]} flex items-center justify-center text-white font-bold text-sm shrink-0 overflow-hidden ${user.status === "inativo" ? "opacity-40" : ""}`}>
                                        {user.avatar.includes('/') || user.avatar.includes('.') ? (
                                            /* eslint-disable-next-line @next/next/no-img-element */
                                            <img src={`${(process.env.NEXT_PUBLIC_API_URL || "http://localhost:7777").replace(/\/$/, '')}/${user.avatar.replace(/^\//, '')}`} alt="" className="w-full h-full object-cover" />
                                        ) : (
                                            user.avatar
                                        )}
                                    </div>
                                ...
                                <div ref={activeMenu === user.id ? menuRef : undefined} className="relative w-10 flex items-center justify-center">
                                    ...
                                    <AnimatePresence>
                                        {activeMenu === user.id && (
                                            ...
                                            >
                                                <Link
                                                    href={`/dashboard/management/settings/users/${user.id}/edit`}

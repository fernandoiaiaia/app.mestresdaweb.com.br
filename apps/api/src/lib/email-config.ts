...
export async function getSystemEmailConfig() {
    try {
        ...
        if (setting) {
            ...
            const fromEmail = meta?.senderEmail || creds?.senderEmail || env.BREVO_FROM_EMAIL || "noreply@mestresdaweb.com.br";
            const fromName = meta?.senderName || creds?.senderName || env.BREVO_FROM_NAME || "ProposalAI";

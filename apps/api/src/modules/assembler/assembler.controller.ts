...
export const assemblerController = {
    ...
    async reviewProposal(req: Request, res: Response) {
        ...
        const data = await AssemblerService.reviewProposal(
            ...
            { action, comment, by: (req.user as any)?.userName || "Manager" }
        );
        res.json({ success: true, data });
    },
    ...
    async connechStatus(req: Request, res: Response) {
        try {
            ...
            if (e.message === "NOT_FOUND") return res.status(404).json({ success: false, error: { code: "NOT_FOUND", message: "Proposta não encontrada." } });
            throw e;
        }
    },

    async publishToConnech(req: Request, res: Response) {
        try {
            const outcome = await AssemblerService.publishToConnech(req.user!.userId, req.params.id as string);

            if (outcome.ok) {
                return res.json({
                    success: true,
                    data: outcome.duplicate
                        ? { duplicate: true, version: outcome.version }
                        : { duplicate: false, opportunityId: outcome.opportunityId, version: outcome.version },
                });
            }

            const statusByCode: Record<string, number> = {
                NOT_CONFIGURED: 503,
                INVALID_SECRET: 502, // não 401 — evita o front tratar como sessão expirada
                DEAL_NOT_FOUND: 404,
                SCOPE_LOCKED: 409,
                INVALID_PAYLOAD: 422,
                NETWORK_ERROR: 502,
            ...
            const httpStatus = statusByCode[outcome.code] ?? 502;
            return res.status(httpStatus).json({
                success: false,
                error: { code: outcome.code, message: outcome.message, issues: (outcome as any).issues },
            });
        } catch (e: any) {
            if (e.message === "NOT_FOUND") return res.status(404).json({ success: false, error: { code: "NOT_FOUND", message: "Proposta não encontrada." } });
            if (e.message === "NO_DEAL_LINKED") return res.status(400).json({ success: false, error: { code: "NO_DEAL_LINKED", message: "Esta proposta não está vinculada a um negócio." } });
            if (e.message === "NOT_CONNECH_DEAL") return res.status(400).json({ success: false, error: { code: "NOT_CONNECH_DEAL", message: "Este negócio não veio do Connech — publicação indisponível." } });
            if (e.name === "EmptyScopeError") return res.status(400).json({ success: false, error: { code: "EMPTY_SCOPE", message: e.message } });
            throw e;
        }
    },

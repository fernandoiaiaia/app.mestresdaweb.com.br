...
export const backupService = {
    ...
    async getSettings(user: JwtUser) {
        return prisma.backupSettings.findFirst();
    },
    ...
    async getStats(user: JwtUser) {
        const [proposalCount, clientCount, backupCount, successCount, failedCount, lastBackup] = await Promise.all([
            ...
            prisma.backupEntry.count({ where: { status: "success" } }),
            prisma.backupEntry.count({ where: { status: "failed" } }),
            prisma.backupEntry.findFirst({ orderBy: { createdAt: "desc" } }),
        ]);

        // Get storage used (scan all user directories in backup since it's global)
        let storageUsed = 0;
        try {
            const users = await fs.readdir(BACKUP_DIR);
            for (const u of users) {
                const userDir = path.join(BACKUP_DIR, u);
                const stat = await fs.stat(userDir);
                if (stat.isDirectory()) {
                    const files = await fs.readdir(userDir);
                    for (const file of files) {
                        const fileStat = await fs.stat(path.join(userDir, file));
    ...
    async deleteEntry(id: string, user: JwtUser) {
        const entry = await prisma.backupEntry.findFirst({ where: { id } });
        if (entry?.filePath) {
            try { await fs.unlink(entry.filePath); } catch { /* file may not exist */ }
        }
        return prisma.backupEntry.deleteMany({ where: { id } });
    },
    ...
    async createBackup(user: JwtUser) {
        ...
        if (incConfig) {
            ...
            data.notificationSettings = await prisma.notificationSettings.findFirst();
    ...
    async exportData(format: "json" | "csv", modules: { proposals?: boolean; clients?: boolean; config?: boolean }, user: JwtUser) {
        ...
        if (modules.config) {
            data.notificationSettings = await prisma.notificationSettings.findFirst();
            data.backupSettings = await prisma.backupSettings.findFirst();
        }
    ...
    async deleteData(modules: { proposals?: boolean; clients?: boolean; backupHistory?: boolean }, user: JwtUser) {
        ...
        if (modules.backupHistory) {
            ...
            const entries = await prisma.backupEntry.findMany({});
            for (const e of entries) {
                if (e.filePath) {
                    try { await fs.unlink(e.filePath); } catch { /* ok */ }
                }
            }
            const { count } = await prisma.backupEntry.deleteMany({});

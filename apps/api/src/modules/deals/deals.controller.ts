...
import { dealsSalesService } from "./deals.sales.service.js";
...
export const dealsController = {
    async list(req: Request, res: Response) {
        const query = {
            ...
            consultantId: typeof req.query.consultantId === 'string' && req.query.consultantId !== 'all' ? req.query.consultantId : undefined,
        };
    ...
    async salesReport(req: Request, res: Response) {
        const query = {
            ...
            funnelId: typeof req.query.funnelId === 'string' && req.query.funnelId !== 'all' ? req.query.funnelId : undefined,
            search: typeof req.query.search === 'string' ? req.query.search : undefined,
        };
        try {
            const data = await dealsSalesService.list({ userId: req.user!.userId, ...query });
            res.json({ success: true, data });
        } catch (error: any) {
            res.status(500).json({ success: false, error: "Erro gerando relatório de vendas" });
        }
    },

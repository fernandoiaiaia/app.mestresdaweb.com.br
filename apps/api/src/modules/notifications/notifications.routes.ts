...
const requireManage = requirePermission("settings.notifications", "manage");

// ═══ Global settings ═══
router.get("/settings", notificationsController.getSettings);
router.put("/settings", requireManage, validate({ body: upsertNotificationSettingsSchema }), notificationsController.upsertSettings);

...
const requireManage = requirePermission("settings.security", "manage");

// Settings
router.get("/", securityController.getSettings);
router.put("/", requireManage, validate({ body: upsertSecuritySettingsSchema }), securityController.upsertSettings);

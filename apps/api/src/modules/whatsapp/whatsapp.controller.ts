...
router.get("/sse", (req: Request, res: Response) => {
    ...
    res.setHeader("X-Accel-Buffering", "no"); // Prevent Caddy/Nginx from buffering
    res.flushHeaders();
...
router.get("/conversations", async (req: Request, res: Response) => {
    try {
        ...
        if (role === "OWNER" || role === "ADMIN") {
            whereClause = {}; // See all conversations
        }
...
router.post("/conversations/init", async (req: Request, res: Response) => {
    try {
        ...
        // 2. Find Conversation
        if (role === "OWNER" || role === "ADMIN") {
            // Admin/owner should first try to find ANY existing conversation for this contact
            const existingConv = await prisma.whatsappConversation.findFirst({
                where: { contactId: contact.id },
                orderBy: { lastMessageAt: "desc" }
            });
            if (existingConv) {
                return res.json({ success: true, data: { id: existingConv.id } });
            }
        }

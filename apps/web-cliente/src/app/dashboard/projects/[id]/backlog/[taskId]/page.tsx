...
import {
    ArrowLeft, Clock, FolderKanban, Tag, CalendarDays,
    AlertTriangle, MessageSquare, Paperclip, History, Send,
    ChevronDown, Bug, Upload, Image as ImageIcon, Video, FileText as FileTextIcon, File,
...
export default function TaskDetailPage() {
    ...
    const [activeTab, setActiveTab] = useState<"tasks" | "details" | "bugs" | "comments" | "history">("tasks");


    // Edit Engine State 
    const [isEditing] = useState(taskId === "new");
    ...
    useEffect(() => {
        ...
        if (taskId !== "new") {
            ...
            api<{ id: string; action: string; description: string; createdAt: string; user: { id: string; name: string } }[]>(`/api/dev-projects/tasks/${taskId}/history`).then(r => { if (r.success && r.data) setHistoryEntries(r.data); });
            api<ApiBug[]>(`/api/dev-projects/tasks/${taskId}/bugs`).then(r => { if (r.success && r.data) setBugEntries(r.data); });
            api<{ id: string; originalName: string; mimeType: string; size: number; createdAt: string }[]>(`/api/dev-projects/tasks/${taskId}/attachments`).then(r => { if (r.success && r.data) setAttachments(r.data); });
        }
    }, [taskId, fetchTask]);
    ...
    const getFileIcon = (type: string) => { if (type?.startsWith("image/")) return <ImageIcon size={14} className="text-blue-400" />; if (type?.startsWith("video/")) return <Video size={14} className="text-purple-400" />; if (type === "application/pdf") return <FileTextIcon size={14} className="text-red-400" />; return <File size={14} className="text-blue-400" />; };
    ...
    if (!task) {
        return (
            <div className="p-8 text-center">
                ...
                <Link href={`/dashboard/projects/${projectId}/backlog`} className="mt-4 inline-flex items-center gap-1.5 text-blue-400 hover:text-blue-300 text-sm">
        ...
        <div className="min-h-screen bg-slate-950">
            ...
            <div className="border-b border-white/[0.06] bg-slate-900/80 backdrop-blur-md sticky top-0 z-10">
                <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        <button onClick={() => setChatOpen(true)} className="px-3 py-1.5 text-xs font-bold bg-slate-800 hover:bg-slate-700 text-white rounded-md transition-colors border border-white/[0.06] flex items-center gap-1.5 shadow-lg shadow-black/20">
            ...
            <div className="max-w-6xl mx-auto px-6 py-8">
                ...
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
                    ...
                    {/* ── Hours summary card (inline, always visible) ── */}
                    <div className="mt-5 flex items-center gap-4 p-4 rounded-xl border bg-slate-800/40 border-slate-700/30 transition-all">
                        {/* Hours summary */}
                                ...
                                <motion.div key="tasks" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="space-y-2">
                                    ...
                                    {checklist.map(item => {
                                        return (
                                            ...
                                            >
                                                {/* Checkbox Static */}
                                                <div className="shrink-0 transition-all mt-0.5">
                                                    {item.done
                                        ...
                                        return (
                                            <div key={bug.id} className={`p-4 rounded-xl border ${bug.status === "fixed" ? "bg-emerald-500/5 border-emerald-500/20" : "bg-slate-900/40 border-slate-700/30"}`}>
                                                ...
                                                <div className="flex items-center justify-between pt-2 border-t border-slate-700/20">
                                                    <span className="text-xs text-slate-500">Relatar</span>
                                ...
                                {comments.length > 0 ? comments.map(c => {
                                    return (
                                        <div key={c.id} className="flex flex-col gap-1">
                                            ...
                                            <div className="ml-8 bg-slate-800/80 border rounded-2xl rounded-tl-sm p-3 inline-block self-start max-w-[90%] transition-colors duration-500 border-slate-700/50">

...
// `story` holds either free-text prose or a JSON-stringified checklist of functionalities
// (written by the Assembler auto-generation flow and the task editor's "Funcionalidades"
// tab) — never render it raw, it must be told apart before display.
function getStoryPreview(story: string | null): { text: string; isChecklist: boolean } | null {
    ...
    if (trimmed.startsWith("[")) {
        try {
            ...
            if (Array.isArray(parsed)) {
                ...
                return { text: `${parsed.length} funcionalidade${parsed.length > 1 ? "s" : ""}`, isChecklist: true };
            }
        } catch {
            // Not valid JSON after all — fall through and show it as plain text.
        }
    }
    return { text: trimmed, isChecklist: false };
}
                                                            ...
                                                            {modTasks.map(task => {
                                                                ...
                                                                return (
                                                                    <div key={task.id} onClick={() => router.push(`/dashboard/projects/${projectId}/backlog/${task.id}`)}
                                                                        ...
                                                                        {(() => {
                                                                            const storyPreview = getStoryPreview(task.story);
                                                                            if (!storyPreview) return null;
                                                                            return (
                                                                                <p className="text-xs text-slate-400 line-clamp-4 mt-2 italic flex-1 break-words leading-loose">
                                                                                    {storyPreview.isChecklist ? storyPreview.text : `"${storyPreview.text}"`}
                                                                                </p>
                                                                            );
                                                                        })()}

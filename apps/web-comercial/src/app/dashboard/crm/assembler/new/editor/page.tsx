...
function EditableText({
  value, onChange, isTextArea = false, className = "", placeholder = ""
}: {
  value: string; onChange: (val: string) => void;
  isTextArea?: boolean; className?: string; placeholder?: string;
}) {
      ...
      <div className="flex gap-2 w-full items-start">
        {isTextArea ? (
          <textarea autoFocus value={tempVal} onChange={e => setTempVal(e.target.value)}
            onBlur={save} rows={3} placeholder={placeholder}
          ...
          <input autoFocus type="text" value={tempVal} onChange={e => setTempVal(e.target.value)}
            onBlur={save} onKeyDown={e => e.key === "Enter" && save()} placeholder={placeholder}
  ...
  return (
    <div className={`group flex items-start gap-2 relative min-w-0 w-full ${className}`}>
      <span className={`min-w-0 flex-1 ${isTextArea ? "whitespace-pre-wrap" : "truncate"}`}>
        {value || (placeholder ? <span className="opacity-50 italic">{placeholder}</span> : "")}
      </span>
...
export default function EditorPage() {
  ...
  const refreshConnechStatus = React.useCallback(async (proposalId: string) => {
    try {
      const status = await getConnechStatus(proposalId);
      setConnechEligible(status.eligible);
    } catch {
      // Sonda apenas — falha silenciosamente
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (draft.id && !draft.id.startsWith("scope_") && !draft.id.startsWith("draft_")) {
      ...
      refreshConnechStatus(draft.id);
    }

    // Check if we should automatically open the feedback panel (e.g. redirected from notification)
    if (localStorage.getItem("proposals_assembler_open_feedback") === "true") {
      setFeedbackPanelOpen(true);
      localStorage.removeItem("proposals_assembler_open_feedback");
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (dbId) void refreshConnechStatus(dbId);
  ...
  const handlePublishToConnech = async () => {
    if (!scope || publishingConnech) return;
    const ok = await confirm({
      title: "Publicar escopo para fornecedores?",
      message: "O escopo (sem horas e sem valores) ficará visível para os fornecedores ativos no Connech responderem.",
      confirmText: "Publicar",
    });
    if (!ok) return;

    setPublishingConnech(true);
    saveScopeDraft(scope);
    try {
      const saved = await saveProposalToList(scope);
      const dbId = saved.id;
      ...
      const result = await publishScopeToConnech(dbId);
      toast.success(result.duplicate
        ? "Escopo já estava atualizado no Connech."
        : "Escopo publicado para os fornecedores!");
      if (dbId) void refreshConnechStatus(dbId);
    } catch (e: unknown) {
      const err = e as Error;
      console.error("Connech Publish Error:", err);
      toast.error(err?.message || "Erro ao publicar escopo.");
    } finally {
      setPublishingConnech(false);
    }
  };
  ...
  const setUserSummary   = (uIdx: number, val: string) => withScope(s => { s.users[uIdx].userSummary = val; return s; });
  ...
  const addPlatform = (uIdx: number) => { const s = { ...scope! }; s.users[uIdx].platforms.push({ id: "p_" + genId(), platformName: "Nova Plataforma", objective: "", modules: [] }); const pIdx = s.users[uIdx].platforms.length - 1; setExpandedNodes(prev => ({ ...prev, [`u${uIdx}`]: true, [`u${uIdx}p${pIdx}`]: true })); updateScope(s); };
  const addModule = (uIdx: number, pIdx: number) => { const s = { ...scope! }; s.users[uIdx].platforms[pIdx].modules.push({ id: "m_" + genId(), title: "Novo Módulo", screens: [] }); const mIdx = s.users[uIdx].platforms[pIdx].modules.length - 1; setExpandedNodes(prev => ({ ...prev, [`u${uIdx}`]: true, [`u${uIdx}p${pIdx}`]: true, [`u${uIdx}p${pIdx}m${mIdx}`]: true })); updateScope(s); };
    ...
    <div className="relative min-h-screen bg-slate-900 text-white selection:bg-blue-500/30 pb-32">
          ...
          <div className="flex items-center gap-4">
            ...
            {connechLinked && (
              <button
                ...
                title="Publicar escopo (sem horas) para os fornecedores no Connech"
                className={`px-5 py-2 rounded-full font-semibold text-sm transition-all flex items-center gap-2 border ${
                  publishingConnech
                    ...
                    : "bg-emerald-500/20 border-emerald-500/40 text-emerald-300 hover:bg-emerald-500/30 shadow-[0_0_15px_rgba(16,185,129,0.2)]"
                }`}
              >
                {publishingConnech ? (
                  <><Loader2 size={14} className="animate-spin" /> Publicando...</>
                ) : (
                  <><Send size={14} /> Publicar para Fornecedores</>
                )}
              </button>
            )}
      ...
      {feedbacks.length > 0 && (
        <button
          ...
          className={`fixed bottom-8 right-8 z-[60] flex items-center gap-2.5 px-5 py-3 rounded-full font-bold text-sm transition-all hover:scale-105 active:scale-95 ${
            unreadCount > 0 
              ? "bg-red-500 hover:bg-red-600 text-white shadow-[0_0_30px_rgba(239,68,68,0.5)] hover:shadow-[0_0_40px_rgba(239,68,68,0.7)]" 
              : "bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700/50 shadow-lg shadow-black/20"
          }`}
        >
                ...
                <div className="flex bg-slate-900/60 rounded-xl p-1 border border-slate-700/40">
                    {[15, 30].map(days => (
                        <button
                            key={days}
                            onClick={() => {
                                const s = { ...scope }; s.validityDays = days; updateScope(s);
                            }}
                            className={`flex-1 py-1.5 text-sm font-medium rounded-lg transition-all ${
                                scope.validityDays === days 
            ...
            <div className="bg-slate-800/40 border border-slate-700/50 rounded-2xl p-6 backdrop-blur-md">
                ...
                <div className="relative">
                    <input
                        type="number"
                        min={0}
                        value={scope.deliveryDeadline ?? ""}
                        onChange={e => {
                            const value = e.target.value;
                            const s = { ...scope };
                            s.deliveryDeadline = value === "" ? undefined : Math.max(0, Number(value));
                            updateScope(s);
                        }}
                        placeholder="Ex: 30"
                        className="w-full bg-slate-900/60 border border-slate-700/40 rounded-xl px-4 py-2 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500 transition-colors"
                    />
                </div>
            </div>
        </div>
            ...
            <div key={user.id + uIdx} className="bg-slate-800/40 border border-slate-700/50 rounded-2xl overflow-hidden backdrop-blur-md">
              ...
              <div className={`p-6 flex items-center gap-4 cursor-pointer hover:bg-white/[0.02] transition-colors ${isUExp ? "border-b border-white/[0.05]" : ""}`} onClick={() => toggleNode(uId)}>
                ...
                <div className="flex-1 min-w-0" onClick={e => e.stopPropagation()}>
                  ...
                  <EditableText value={user.userSummary || ""} onChange={val => setUserSummary(uIdx, val)} placeholder="Adicionar resumo sobre o usuário..." className="text-sm text-[#86868b] mt-2" isTextArea />
                </div>
              ...
              {isUExp && (
                <div className="p-6 pt-2 space-y-4">
                  ...
                  <div className="pt-2 border-t border-white/5">
                    <button onClick={() => addPlatform(uIdx)} className="text-sm font-medium text-[#86868b] hover:text-white flex items-center gap-2 bg-white/5 hover:bg-white/10 px-4 py-2.5 rounded-xl transition-colors">
        ...
        <div className="fixed inset-0 z-[70] flex">
          ...
          <div className="w-full max-w-[480px] bg-slate-900 border-l border-slate-700/50 shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
              ...
              .map(g => {
                const screenUnreadIds = g.comments.filter(c => !c.read).map(c => c.id);
                const hasUnread = screenUnreadIds.length > 0;
                return (
                  <div key={g.screenId} onClick={() => { navigateToScreen(g.screenId, g.screenTitle); markBatchAsRead(screenUnreadIds); }}
                    className={`p-4 rounded-xl border shadow-[0_0_10px_rgba(239,68,68,0.05)] hover:bg-slate-800 hover:border-red-500/40 cursor-pointer transition-colors group ${
                      hasUnread ? "bg-red-500/10 border-red-500/30" : "bg-slate-800/40 border-slate-700/50"
                    }`}>
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <Layout size={14} className={hasUnread ? "text-red-400" : "text-slate-500"} />
                        <div>
                          <p className={`text-sm font-bold transition-colors truncate max-w-[200px] ${hasUnread ? "text-white group-hover:text-red-400" : "text-slate-300 group-hover:text-white"}`}>{g.screenTitle}</p>
                          <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">{g.moduleName}</p>
                        </div>
                      </div>
                      <span className={`flex items-center justify-center min-w-[24px] h-6 px-1.5 rounded-full text-[10px] font-black border ${hasUnread ? "bg-red-600/20 text-red-400 border-red-500/30" : "bg-slate-700/50 text-slate-400 border-slate-600/50"}`}>
                        {g.comments.length}
                      </span>
                    </div>
                    <div className="pl-6">

...
import { Plus, Package, Clock, Users, ArrowRight, Trash2, Search, Layers, Loader2, CheckCircle, X, Sparkles, Copy } from "lucide-react";
...
export default function AssemblerListingPage() {
    ...
    const handleOpen = (scope: CompleteScope) => {
        if (typeof window !== "undefined") {
            // Strip rawMeta (DB wrapper) and clientFeedback before saving to localStorage
    ...
    const handleDuplicate = (scope: CompleteScope, e: React.MouseEvent) => {
        e.stopPropagation();
        if (typeof window !== "undefined") {
            // eslint-disable-next-line @typescript-eslint/no-unused-vars
            const { rawMeta, id, createdAt, ...cleanScope } = scope as any;
            
            const duplicatedScope = {
                ...cleanScope,
                id: `draft_${Math.random().toString(36).substring(2, 11)}`,
                title: cleanScope.title ? `[Cópia] ${cleanScope.title}` : "[Cópia] Nova Proposta",
                projectSummary: cleanScope.projectSummary ? `[Cópia] ${cleanScope.projectSummary}` : "[Cópia] Novo Projeto"
            };

            localStorage.setItem("proposals_assembler_current_scope", JSON.stringify(duplicatedScope));
            router.push("/dashboard/crm/assembler/new/editor");
        }
    };

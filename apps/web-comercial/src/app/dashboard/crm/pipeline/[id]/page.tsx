...
export default function PipelineDealDetail() {
    ...
    const loadDeal = async (id: string, background = false) => {
        try {
            ...
            const [dealResponse, funnelsResponse, lossRes, sourcesRes] = await Promise.all([
                api<DealDetail>(`/api/deals/${id}`, { method: "GET" }),
                api<Funnel[]>('/api/funnels', { method: "GET" }),
                api<LossReason[]>('/api/loss-reasons', { method: "GET" }),
                api<{ name: string; active: boolean }[]>('/api/sources', { method: "GET" }),
            ]);
            ...
            if (sourcesRes.success && sourcesRes.data) {
                setSourceOptions(sourcesRes.data.filter(s => s.active !== false).map(s => s.name));
            }
        } catch (error) {
                            ...
                            <div className="absolute right-0 top-full mt-1 w-48 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-lg shadow-xl z-50">
                                {user?.role === 'OWNER' ? (
                                    ...
                                    <div className="px-4 py-2.5 text-xs text-slate-400 text-center italic">
                                        Apenas proprietários podem excluir negócios.
                                    </div>
                                )}
                            </div>
                        )}
                                ...
                                {editingField === "source" ? (
                                    <div ref={sourceDropdownRef} className="flex-1 relative -ml-2">
                                        ...
                                        <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-lg shadow-xl max-h-48 overflow-y-auto">
                                            {sourceOptions.filter(s => s.toLowerCase().includes(sourceSearch.toLowerCase())).map(s => (
                                                <button
                                                    key={s}
                                            ...
                                            {sourceOptions.filter(s => s.toLowerCase().includes(sourceSearch.toLowerCase())).length === 0 && (
                            ...
                            <div className="space-y-4">
                                <div className="flex flex-col gap-1.5">
                                    ...
                                    <div className="grid grid-cols-1 gap-2 max-h-[300px] overflow-y-auto custom-scrollbar pr-1 -mr-1">
                                        {(() => {
                                            // Only show loss reasons assigned to this specific stage
                                            const displayReasons = lossReasons.filter(lr => lr.funnelId === deal.funnel?.id && lr.stageId === deal.stageId);
                                            ...
                                            if (displayReasons.length === 0) {
                                                return (
                                                    <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-white/5 text-center">
                                                        <p className="text-sm text-slate-500">Nenhum motivo de perda cadastrado para a etapa <strong>{deal.stage?.name}</strong>.</p>

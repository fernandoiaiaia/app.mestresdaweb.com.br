...
import { Kanban, Plus, Search, Filter, MoreHorizontal, User, Users, DollarSign, Calendar, Clock, ArrowRight, Eye, Edit3, Trash2, ChevronDown, Phone, Mail, Building2, GripVertical, X, MessageSquare, Send, FileText, Tag } from "lucide-react";
...
export default function PipelinePage() {
    ...
    useEffect(() => {
        loadFunnels();
        if (isManagerOrOwner) {
            api<{ id: string; name: string; role: string }[]>('/api/users', { method: "GET" }).then(res => {
                if (res.data) setTeamMembers(res.data);
            });
        }
    }, []);

    // Reload deals when consultant filter changes
    useEffect(() => {
        if (selectedFunnel) selectFunnel(selectedFunnel);
    }, [selectedConsultant]);
        ...
        <div className="p-6 md:p-8">
            ...
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
                <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        {/* Consultant Filter (Owner/Manager only) */}
                        {isManagerOrOwner && teamMembers.length > 0 && (
                            <div className="relative">
                                ...
                                <select
                                    ...
                                    className="appearance-none pl-9 pr-10 py-2 bg-slate-800/50 border border-white/[0.08] rounded-xl text-sm text-white focus:outline-none focus:border-blue-500/40 font-medium cursor-pointer"
                                >
                                    <option value="all" className="bg-slate-900 text-white">Todos os responsáveis</option>
                                    {teamMembers.map(m => (
                                ...
                                {filteredDeals.map((deal) => (
                                    ...
                                    >
                                        <div className="flex items-start justify-between mb-2">
                                            ...
                                            <div className="relative" onMouseLeave={() => setOpenMenuId(null)}>
                                                <button 
                                                    ...
                                                    className="p-0.5 text-slate-700 hover:text-white opacity-0 group-hover:opacity-100 transition-all"
                                                >
                                                    <MoreHorizontal size={14} />
                                                </button>
                                                {openMenuId === deal.id && (
                                                    <div className="absolute right-0 top-full mt-1 w-40 bg-slate-800 border border-white/10 rounded-lg shadow-xl z-50">
                                                        {user?.role === 'OWNER' ? (
                                                            <button
                                                                onClick={(e) => { e.stopPropagation(); setOpenMenuId(null); handleDeleteDeal(deal.id); }}
                                                                className="w-full flex items-center gap-2 px-3 py-2 text-xs text-red-400 hover:bg-red-500/10 transition-colors rounded-lg text-left"
                                                            >
                                                                <Trash2 size={12} /> Excluir
                                                            </button>
                                                        ) : (
                                                            <div className="px-3 py-2 text-[10px] text-slate-400 text-center italic leading-tight">
                                                                Apenas proprietários podem excluir.
                                                            </div>
                                                        )}
                                                    </div>
                                                )}
                                        ...
                                        <div className="flex items-center gap-1.5 mb-2">
                                            ...
                                            <span className="text-[11px] text-slate-400">{deal.source || "Desconhecida"}</span>
                                        </div>
                                        ...
                                        <div className="flex items-center justify-between text-[10px] text-slate-600">
                                            ...
                                            <span className="flex items-center gap-1"><Clock size={10} /> {deal.createdAt ? differenceInCalendarDays(new Date(), parseISO(deal.createdAt)) : 0}d</span>
                                        </div>
                                    </div>
                                ))}
                                ...
                                <div className="space-y-5">
                                    ...
                                    <div className="space-y-3">
                                        {[
                                            ...
                                            { icon: Clock, label: "Dias no funil", value: `${selectedDeal.createdAt ? differenceInCalendarDays(new Date(), parseISO(selectedDeal.createdAt)) : 0} dias` },
                                        ].map((item) => (

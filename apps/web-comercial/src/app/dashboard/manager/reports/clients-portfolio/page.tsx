...
export default function ClientsPortfolioPage() {
    ...
    const [data, setData] = useState<{
        metrics: { totalClientes: number; ativos: number; novos: number; inativos: number };
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const result = await api<any>("/api/deals/reports/clients-portfolio");
                if (result.success && result.data) setData(result.data);
            } catch (err) {
                console.error("Falha ao carregar carteira de clientes", err);
            } finally {
                setIsLoading(false);
            }
        };
        ...
        <div className="p-6 md:p-10 max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                {isLoading || !data ? (
                    ...
                    [   
                        { label: "Total Clientes", value: data.metrics.totalClientes.toString(), color: "text-cyan-400" }, 
                        { label: "Clientes Ativos", value: data.metrics.ativos.toString(), color: "text-blue-400" }, 
                        { label: "Novos (30d)", value: data.metrics.novos.toString(), color: "text-blue-400" }, 
                        { label: "Inativos", value: data.metrics.inativos.toString(), color: "text-red-400" }
                    ].map((k, i) => (
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-slate-800/40 border border-white/[0.06] rounded-2xl p-6">
                    ...
                    <div className="h-[220px]">
                        {isLoading || !data ? (
                            ...
                            <div className="text-xs text-slate-500 text-center py-10">Nenhum segmento categorizado.</div>
                        ) : (
                            <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={data.bySegment} cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={3} dataKey="value" stroke="none">{data.bySegment.map((e, i) => <Cell key={i} fill={e.color} />)}</Pie><Tooltip contentStyle={ts} /></PieChart></ResponsiveContainer>
                        )}
                    </div>
                    <div className="space-y-2 mt-2">
                        {data && data.bySegment.length > 0 && data.bySegment.map(s => (<div key={s.name} className="flex items-center gap-2 text-[11px]"><div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: s.color }} /><span className="text-slate-400 flex-1">{s.name}</span><span className="text-white font-bold">{s.value}%</span></div>))}
                    </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="bg-slate-800/40 border border-white/[0.06] rounded-2xl p-6">
                    ...
                    <div className="h-[280px]">
                        {isLoading || !data ? (
                            ...
                            <div className="text-xs text-slate-500 text-center py-10">Nenhum dado de origem.</div>
                        ) : (
                            <ResponsiveContainer width="100%" height="100%"><BarChart data={data.bySource} layout="vertical"><CartesianGrid strokeDasharray="3 3" stroke="#1e293b" horizontal={false} /><XAxis type="number" stroke="#475569" fontSize={11} tickLine={false} axisLine={false} /><YAxis type="category" dataKey="source" stroke="#475569" fontSize={11} tickLine={false} axisLine={false} width={110} /><Tooltip contentStyle={ts} /><Bar dataKey="clientes" fill="#06b6d4" radius={[0, 4, 4, 0]} name="Clientes" /></BarChart></ResponsiveContainer>
                        )}
                    </div>
                </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="bg-slate-800/40 border border-white/[0.06] rounded-2xl overflow-hidden">
                ...
                <table className="w-full text-left"><thead><tr className="border-b border-white/[0.04] bg-slate-900/50">
                    ...
                    {isLoading || !data ? (
                        <tr><td colSpan={5} className="text-center py-6 text-sm text-slate-500">Consultando topologia de faturamento...</td></tr>
                    ) : data.topClients.length === 0 ? (
                        <tr><td colSpan={5} className="text-center py-6 text-sm text-slate-500">Ainda não há geração de receita no histórico e contatos.</td></tr>
                    ) : (

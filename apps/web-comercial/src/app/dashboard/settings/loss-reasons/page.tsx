...
export default function LossReasonsPage() {
    ...
    return (
        <div className="p-6 md:p-10 max-w-5xl mx-auto">
            ...
            {/* Reasons List — Grouped by Funnel → Stage */}
            {funnels.length === 0 && reasons.length === 0 && (
                <div className="text-center py-12 text-slate-600">
                    <ThumbsDown size={32} className="mx-auto mb-2 opacity-20" />
                    <p className="text-sm">Nenhum motivo cadastrado ainda</p>
                </div>
            )}

            {funnels.map((funnel, fi) => {
                const funnelReasons = filtered.filter(r => r.funnel?.id === funnel.id);
                if (funnelReasons.length === 0 && searchQuery) return null;

                return (
                    <motion.div key={funnel.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: fi * 0.05 }} className="mb-8">
                        {/* Funnel header */}
                        <div className="flex items-center gap-2 mb-3">
                            <TrendingDown size={14} className="text-slate-500" />
                            <h2 className="text-sm font-bold text-white tracking-wide">{funnel.name}</h2>
                            <span className="text-[10px] text-slate-600 bg-slate-800/60 px-2 py-0.5 rounded-full">{funnelReasons.length} motivo{funnelReasons.length !== 1 ? 's' : ''}</span>
                        </div>

                        {/* Stages */}
                        <div className="space-y-3 pl-2 border-l-2 border-white/[0.04] ml-1.5">
                            {funnel.stages.map((stage) => {
                                const stageReasons = funnelReasons.filter(r => r.stage?.id === stage.id);

                                // Skip empty stages when searching
                                if (stageReasons.length === 0 && searchQuery) return null;
                                // Skip stages not matching filter
                                if (filterCategory !== "all" && filterCategory !== stage.id) return null;

                                return (
                                    <div key={stage.id} className="pl-4">
                                        {/* Stage header */}
                                        <div className="flex items-center gap-2 mb-2">
                                            <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: stage.color || '#64748b' }} />
                                            <span className="text-xs font-semibold" style={{ color: stage.color || '#94a3b8' }}>{stage.name}</span>
                                            <span className="text-[10px] text-slate-600">{stageReasons.length} motivo{stageReasons.length !== 1 ? 's' : ''}</span>
                                            <button 
                                                onClick={() => { 
                                                    setEditId(null); 
                                                    setFormName(""); 
                                                    setFormDesc(""); 
                                                    setFormFunnelId(funnel.id); 
                                                    setFormStageId(stage.id); 
                                                    setShowModal(true); 
                                                }} 
                                                className="ml-auto flex items-center gap-1 text-[10px] text-slate-600 hover:text-blue-400 transition-colors px-2 py-0.5 rounded-lg hover:bg-blue-500/10"
                                            >
                                        ...
                                        {stageReasons.length === 0 ? (
                                            <div className="py-3 px-4 text-[11px] text-slate-600 italic bg-slate-800/20 rounded-lg border border-dashed border-white/[0.04]">
                                                Nenhum motivo cadastrado para esta etapa
                                            </div>
                                        ) : (
                                            <div className="space-y-1.5">
                                                {stageReasons.map((reason, i) => {
                                                    const barW = totalUsage > 0 ? Math.max((reason.usageCount / (topReason?.usageCount || 1)) * 100, 2) : 2;
                                                    return (
                                                        <motion.div key={reason.id} initial={{ opacity: 0, x: -5 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.02 }} className={`flex items-center gap-3 p-3 bg-slate-800/40 border border-white/[0.06] rounded-xl hover:border-white/[0.1] transition-colors group ${!reason.active ? 'opacity-50' : ''}`}>
                                                            {/* Name & Description */}
                                                            <div className="flex-1 min-w-0 flex flex-col gap-0.5">
                                                                <span className="text-sm font-medium text-white block truncate">{reason.name}</span>
                                                                {reason.description && <span className="text-[11px] text-slate-500 block truncate">{reason.description}</span>}
                                                            </div>

                                                            {/* Usage bar */}
                                                            <div className="hidden md:block w-24 shrink-0">
                                                                <div className="h-3.5 bg-slate-900/50 rounded-full overflow-hidden">
                                                                    <div className="h-full rounded-full transition-all flex items-center justify-end pr-1" style={{ width: `${barW}%`, minWidth: '14px', backgroundColor: `${stage.color || '#64748b'}60` }}>
                                                                        <span className="text-[7px] font-bold text-white/80">{reason.usageCount}</span>
                                                                    </div>
                                                                </div>
                                                            </div>

                                                            {/* Last used */}
                                                            <span className="text-[10px] text-slate-600 w-20 text-right shrink-0 hidden lg:block">
                                                            ...
                                                            <div className="flex items-center gap-0.5 shrink-0">
                                                                <button onClick={() => toggleActive(reason.id)} className={`p-1.5 rounded-lg transition-colors ${reason.active ? 'text-blue-400 hover:bg-blue-500/10' : 'text-slate-600 hover:bg-white/5'}`}>
                                                                    {reason.active ? <ToggleRight size={15} /> : <ToggleLeft size={15} />}
                                                                </button>
                                                                <button onClick={() => openEdit(reason)} className="p-1.5 rounded-lg text-slate-600 hover:text-white hover:bg-white/5 transition-colors opacity-0 group-hover:opacity-100"><Edit3 size={13} /></button>
                                                                <button onClick={() => setDeleteConfirm(reason.id)} className="p-1.5 rounded-lg text-slate-600 hover:text-red-400 hover:bg-red-500/10 transition-colors opacity-0 group-hover:opacity-100"><Trash2 size={13} /></button>
                                                            </div>
                                                        </motion.div>
                                                    );
                                                })}
            ...
            {/* Orphan reasons (no funnel/stage) */}
            {(() => {
                const orphans = filtered.filter(r => !r.funnel?.id || !funnels.some(f => f.id === r.funnel?.id));
                if (orphans.length === 0) return null;
                return (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-8">
                        <div className="flex items-center gap-2 mb-3">
                            <ThumbsDown size={14} className="text-slate-600" />
                            <h2 className="text-sm font-bold text-slate-500">Sem Funil Atribuído</h2>
                        </div>
                        <div className="space-y-1.5 pl-6">
                            {orphans.map((reason) => (
                                <div key={reason.id} className={`flex items-center gap-3 p-3 bg-slate-800/40 border border-white/[0.06] rounded-xl group ${!reason.active ? 'opacity-50' : ''}`}>
                                    <div className="flex-1 min-w-0"><span className="text-sm font-medium text-white block truncate">{reason.name}</span></div>
                                    <div className="flex items-center gap-0.5 shrink-0">
                                        <button onClick={() => openEdit(reason)} className="p-1.5 rounded-lg text-slate-600 hover:text-white hover:bg-white/5 transition-colors"><Edit3 size={13} /></button>
                                        <button onClick={() => setDeleteConfirm(reason.id)} className="p-1.5 rounded-lg text-slate-600 hover:text-red-400 hover:bg-red-500/10 transition-colors"><Trash2 size={13} /></button>
                                    </div>
                                </div>
                            ))}
                        </div>

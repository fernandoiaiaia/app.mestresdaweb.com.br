...
interface ApiCategory { id: string; name: string; color: string; }
...
export default function EditProductPage() {
        ...
        (async () => {
            try {
                const [catRes, prodRes] = await Promise.all([
                    api<ApiCategory[]>("/api/products/categories", { method: "GET" }),
                    api<any>(`/api/products/${productId}`, { method: "GET" }),
                ]);
                ...
                if (prodRes.success && prodRes.data) {
                    ...
                    setType(p.type || "servico");
                    setCategoryId(p.categoryId || "");
                    setPriceMin(p.priceMin ? String(p.priceMin) : "");
                    setPriceMax(p.priceMax ? String(p.priceMax) : "");
                    setBillingModel(p.billingModel || "unico");
                    setSetupFee(p.setupFee ? String(p.setupFee) : "");
                    setMarginPercent(p.marginPercent ? String(p.marginPercent) : "");
                    setDiscountMax(p.discountMax ? String(p.discountMax) : "");
                    setWarrantyMonths(p.warrantyMonths != null ? String(p.warrantyMonths) : "3");
                    setComplexity(p.complexity || "medium");
                    setEstimatedHours(p.estimatedHours ? String(p.estimatedHours) : "");
                    setSlaResponse(p.slaResponse || "");
                    setSlaResolution(p.slaResolution || "");
                    const dels = Array.isArray(p.deliverables) ? p.deliverables.map((d: string) => ({ id: uid(), name: d })) : [{ id: uid(), name: "" }];
                    setDeliverables(dels.length > 0 ? dels : [{ id: uid(), name: "" }]);
                    setSelectedTech(Array.isArray(p.techStack) ? p.techStack : []);
                    setActive(p.active ?? true);
                    setShowInProposals(p.showInProposals ?? true);
    ...
    const toggleTech = (tech: string) => setSelectedTech(prev => prev.includes(tech) ? prev.filter(t => t !== tech) : [...prev, tech]);

    const filteredTechGroups = techStackOptions.map(group => ({
        ...group, items: group.items.filter(item => item.toLowerCase().includes(techSearch.toLowerCase())),
    ...
    const handleSave = async () => {
        ...
        try {
            const body = {
                ...
                active, showInProposals, featured,
            };
            const res = await api(`/api/products/${productId}`, { method: "PUT", body });
            if (res.success) router.push("/dashboard/settings/products");
        } catch (e) { console.error(e); }
        ...
        <div className="p-6 md:p-10 max-w-5xl mx-auto pb-32">
            ...
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
                <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-slate-700">/</span>
                    <span className="text-slate-300 text-sm font-medium">Editar Item</span>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-white tracking-tight">Editar Produto ou Serviço</h1>
                        <p className="text-sm text-slate-400">Atualize as informações do item</p>
                    </div>
                </div>
            </motion.div>
            ...
            <div className="space-y-8">
                ...
                <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="bg-slate-800/40 border border-white/[0.06] rounded-2xl overflow-hidden">
                    ...
                    <div className="p-6 space-y-5">
                        <div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                {(Object.keys(typeConfig) as ProductType[]).map(t => {
                                    const tc = typeConfig[t]; const selected = type === t;
                                    return (
                                        <button key={t} type="button" onClick={() => setType(t)} className={`p-3 rounded-xl text-left transition-all border ${selected ? tc.color : 'border-white/[0.06] hover:border-white/[0.12]'}`}>
                                            <div className="flex items-center gap-2 mb-1"><span className="text-lg">{tc.emoji}</span><span className={`text-xs font-bold ${selected ? '' : 'text-slate-400'}`}>{tc.label}</span></div>
                        ...
                        <div>
                            ...
                            <textarea rows={2} value={description} onChange={e => setDescription(e.target.value)} placeholder="texto para proposta..." className="w-full px-4 py-2.5 bg-slate-800/50 border border-white/[0.08] rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500/40 resize-none" />
                        </div>
                        <div>
                            ...
                            <textarea rows={4} value={detailedDescription} onChange={e => setDetailedDescription(e.target.value)} placeholder="Detalhes técnicos..." className="w-full px-4 py-2.5 bg-slate-800/50 border border-white/[0.08] rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500/40 resize-none" />
                        </div>
                        <div>
                ...
                <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-slate-800/40 border border-white/[0.06] rounded-2xl overflow-hidden">
                    <div className="px-6 py-4 border-b border-white/[0.04] flex items-center gap-2"><DollarSign size={16} className="text-blue-400" /><h2 className="text-sm font-bold text-white uppercase tracking-wider">Precificação</h2></div>
                    <div className="p-6 space-y-5">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {[{ label: "Preço Mínimo (R$)", value: priceMin, set: setPriceMin }, { label: "Preço Máximo (R$)", value: priceMax, set: setPriceMax }, { label: "Taxa de Setup (R$)", value: setupFee, set: setSetupFee }].map(f => (
                                <div key={f.label}><label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block mb-1.5">{f.label}</label><div className="relative"><span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500 text-xs">R$</span><input type="number" min="0" step="0.01" value={f.value} onChange={e => f.set(e.target.value)} placeholder="0,00" className="w-full pl-10 pr-4 py-2.5 bg-slate-800/50 border border-white/[0.08] rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500/40" /></div></div>
                            ))}
                        </div>
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {(Object.entries(billingLabels) as [BillingModel, string][]).map(([key, label]) => (
                                    <button key={key} type="button" onClick={() => setBillingModel(key)} className={`px-3 py-2 rounded-xl text-xs font-medium border transition-all ${billingModel === key ? 'text-blue-400 bg-blue-500/10 border-blue-500/20' : 'border-white/[0.06] text-slate-500 hover:text-white'}`}>{label}</button>
                                ))}
                            </div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div><label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block mb-1.5">Margem de Lucro (%)</label><input type="number" min="0" max="100" value={marginPercent} onChange={e => setMarginPercent(e.target.value)} placeholder="30" className="w-full px-4 py-2.5 bg-slate-800/50 border border-white/[0.08] rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500/40" /></div>
                            <div><label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block mb-1.5">Desconto Máximo (%)</label><input type="number" min="0" max="100" value={discountMax} onChange={e => setDiscountMax(e.target.value)} placeholder="15" className="w-full px-4 py-2.5 bg-slate-800/50 border border-white/[0.08] rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500/40" /></div>
                            <div><label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block mb-1.5">Garantia (meses)</label><input type="number" min="0" value={warrantyMonths} onChange={e => setWarrantyMonths(e.target.value)} placeholder="3" className="w-full px-4 py-2.5 bg-slate-800/50 border border-white/[0.08] rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500/40" /></div>
                        </div>
                    </div>
                </motion.section>
                ...
                <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="bg-slate-800/40 border border-white/[0.06] rounded-2xl overflow-hidden">
                    <div className="px-6 py-4 border-b border-white/[0.04] flex items-center gap-2"><Clock size={16} className="text-amber-400" /><h2 className="text-sm font-bold text-white uppercase tracking-wider">Escopo & SLA</h2></div>
                    <div className="p-6 space-y-5">
                        <div>
                            <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block mb-2">Complexidade</label>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                {complexityLevels.map(level => {
                                    const selected = complexity === level.value; return (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div><label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block mb-1.5">Horas Estimadas</label><input type="number" min="0" value={estimatedHours} onChange={e => setEstimatedHours(e.target.value)} placeholder="80" className="w-full px-4 py-2.5 bg-slate-800/50 border border-white/[0.08] rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500/40" /></div>
                            <div><label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block mb-1.5">SLA — Resposta</label><input type="text" value={slaResponse} onChange={e => setSlaResponse(e.target.value)} placeholder="Ex: 4 horas úteis" className="w-full px-4 py-2.5 bg-slate-800/50 border border-white/[0.08] rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500/40" /></div>
                            <div><label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block mb-1.5">SLA — Resolução</label><input type="text" value={slaResolution} onChange={e => setSlaResolution(e.target.value)} placeholder="Ex: 24 horas úteis" className="w-full px-4 py-2.5 bg-slate-800/50 border border-white/[0.08] rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500/40" /></div>
                        </div>
                        <div>
                            <div className="flex items-center justify-between mb-2"><label className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Entregas / Deliverables</label><button type="button" onClick={addDeliverable} className="flex items-center gap-1 text-blue-400 hover:text-blue-300 text-xs font-medium transition-colors"><Plus size={12} /> Adicionar</button></div>
                            <div className="space-y-2">
                                {deliverables.map((d, idx) => (
                                    <div key={d.id} className="flex items-center gap-2">
                                        ...
                                        {deliverables.length > 1 && <button type="button" onClick={() => removeDeliverable(d.id)} className="p-1.5 rounded-lg text-slate-600 hover:text-red-400 hover:bg-red-500/10 transition-colors"><Trash2 size={14} /></button>}
                                    </div>
                                ))}
                            </div>
                        </div>
                ...
                <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-slate-800/40 border border-white/[0.06] rounded-2xl overflow-hidden">
                    <div className="px-6 py-4 border-b border-white/[0.04] flex items-center gap-2">
                        <Layers size={16} className="text-indigo-400" /><h2 className="text-sm font-bold text-white uppercase tracking-wider">Stack Tecnológica</h2>
                        {selectedTech.length > 0 && <span className="text-[10px] text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 px-2 py-0.5 rounded-full font-bold">{selectedTech.length} selecionadas</span>}
                    </div>
                    <div className="p-6 space-y-4">
                        <div className="relative"><SearchIcon size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" /><input type="text" value={techSearch} onChange={e => setTechSearch(e.target.value)} placeholder="Buscar tecnologia..." className="w-full pl-9 pr-4 py-2 bg-slate-800/50 border border-white/[0.08] rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-indigo-500/40" /></div>
                        {selectedTech.length > 0 && <div className="flex flex-wrap gap-1.5">{selectedTech.map(tech => (<span key={tech} className="flex items-center gap-1 px-2.5 py-1 bg-indigo-500/10 text-indigo-300 rounded-lg text-xs font-medium border border-indigo-500/20">{tech}<button type="button" onClick={() => toggleTech(tech)} className="hover:text-white"><X size={12} /></button></span>))}</div>}
                        <div className="space-y-4 max-h-[320px] overflow-y-auto pr-1">
                            {filteredTechGroups.map(group => (<div key={group.group}><label className="text-[9px] font-bold uppercase tracking-widest text-slate-600 block mb-1.5">{group.group}</label><div className="flex flex-wrap gap-1.5">{group.items.map(item => { const selected = selectedTech.includes(item); return (<button key={item} type="button" onClick={() => toggleTech(item)} className={`px-2.5 py-1.5 rounded-lg text-xs font-medium border transition-all ${selected ? 'text-indigo-300 bg-indigo-500/15 border-indigo-500/30' : 'text-slate-500 border-white/[0.06] hover:text-white hover:bg-white/5'}`}>{selected && <CheckCircle2 size={10} className="inline mr-1" />}{item}</button>); })}</div></div>))}
                        </div>
                    </div>
                </motion.section>
                ...
                <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="bg-slate-800/40 border border-white/[0.06] rounded-2xl overflow-hidden">
                    <div className="px-6 py-4 border-b border-white/[0.04] flex items-center gap-2"><Package size={16} className="text-blue-400" /><h2 className="text-sm font-bold text-white uppercase tracking-wider">Visibilidade & Status</h2></div>
                    <div className="p-6">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {[
                                ...
                                <button key={toggle.label} type="button" onClick={toggle.onChange} className={`p-4 rounded-xl border text-left transition-all ${toggle.value ? 'bg-blue-500/5 border-blue-500/20' : 'border-white/[0.06]'}`}>
                                    <div className="flex items-center justify-between mb-1">
                                        ...
                                        <div className={`w-8 h-[18px] rounded-full transition-all relative ${toggle.value ? 'bg-blue-500' : 'bg-slate-700'}`}><div className={`w-3.5 h-3.5 rounded-full bg-white absolute top-[2px] transition-all ${toggle.value ? 'left-[17px]' : 'left-[2px]'}`} /></div>
                                    </div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="fixed bottom-0 left-0 right-0 bg-slate-900/95 backdrop-blur-xl border-t border-white/[0.08] p-4 z-50">
                <div className="max-w-5xl mx-auto flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        <Link href="/dashboard/settings/products" className="px-4 py-2.5 text-sm text-slate-400 hover:text-white rounded-xl hover:bg-white/5 transition-colors flex items-center gap-2"><X size={14} /> Cancelar</Link>
                        <button onClick={handleSave} disabled={!isValid || saving} className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 hover:bg-blue-500 disabled:opacity-40 disabled:hover:bg-blue-600 text-white rounded-xl font-semibold text-sm transition-all shadow-lg shadow-blue-600/20">
                            {saving ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />} {saving ? "Salvando..." : "Salvar Alterações"}
                        </button>
                    </div>
                </div>
            </motion.div>

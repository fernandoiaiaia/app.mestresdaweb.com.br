...
interface PresentationPlatform { id: string; title: string; icon: string; count: number; userSummary?: string; modules: PresentationModule[]; }
                ...
                user.platforms.filter(p => p.modules.length > 0).map((plat, pIdx) => {
                    ...
                    return (
                        <div key={`${uIdx}-${pIdx}`} style={{ pageBreakBefore: 'always' }}>
                            ...
                            {pIdx === 0 && user.userSummary && (
                                <p style={{
                                    fontSize: '9.5pt', color: ADS.muted, margin: '-10px 0 18px',
                                    fontWeight: 300, lineHeight: 1.6, whiteSpace: 'pre-wrap'
                                }}>
                                    {user.userSummary}
                                </p>
                            )}
...
function PresentationPageInner() {
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'scope' | 'team' | 'tracking' | 'investment'>('overview');
        ...
        const fetchScope = async () => {
            ...
            return user.platforms.map((plat, pIdx) => {
                ...
                return {
                    ...
                    count: screenCount,
                    userSummary: pIdx === 0 ? user.userSummary : undefined,
                    modules: mods
                };
            }).filter(p => p.count > 0);
        });
    ...
    const estimatedMonths = totalHours > 0 ? Math.ceil(totalHours / 160) : 0;
    const timeline =
        scope.deliveryDeadline && scope.deliveryDeadline > 0
            ? `${scope.deliveryDeadline} dias úteis`
            : estimatedMonths > 0
            ...
            <header className="relative z-50 shrink-0 flex items-center justify-between px-6 lg:px-12 py-6 border-b border-slate-700/50 bg-slate-900/80 backdrop-blur-2xl">
                ...
                <div className="hidden lg:flex items-center bg-slate-800/60 rounded-full p-1 border border-slate-700/50 backdrop-blur-md">
                    {[
                        ...
                        <button key={tab.id} onClick={() => setActiveTab(tab.id as 'overview' | 'scope' | 'team' | 'tracking' | 'investment')}
                            ...
                            >
                                ...
                                <div className="relative z-10 flex flex-col items-center text-center max-w-4xl w-full gap-8">
                                    ...
                                    >
                                        ...
                                        {/* Summary */}
                                        {scope.projectSummary && (
                                            <p className="text-[15px] md:text-[17px] text-[#86868b] font-light leading-relaxed max-w-2xl whitespace-pre-wrap">
                                                {scope.projectSummary}
                                            </p>
                                        )}
                                    </motion.div>

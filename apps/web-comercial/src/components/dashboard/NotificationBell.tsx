...
interface Notification {
    ...
    proposalId?: string;
    dealId?: string;
    metadata?: { proposalId?: string; [key: string]: unknown };
    createdAt: string;
}
...
export default function NotificationBell() {
    ...
    const handleNotificationClick = async (n: Notification) => {
        ...
        if (!n.read) {
            await api(`/api/notifications/feed/${n.id}/read`, { method: "PATCH" });
        ...
        // Resolve proposalId from metadata or top-level field
        const proposalId = n.metadata?.proposalId || n.proposalId;

        // Navigation logic
        if (proposalId) {
            try {
                // Fetch proposal data to inject into editor's localStorage
                const res = await api<any>(`/api/assembler/proposals/${proposalId}`);
                if (res.success && res.data) {
                    let sData = res.data.scopeData || {};
                    if (sData.scopeData) sData = sData.scopeData; // Just in case of double nesting
                    
                    const cleanScope = { ...sData, id: res.data.id, createdAt: res.data.createdAt };
                    delete cleanScope.rawMeta;
                    delete cleanScope.clientFeedback; // Don't pollute localStorage with feedback data
                    localStorage.setItem("proposals_assembler_current_scope", JSON.stringify(cleanScope));
                    
                    if (n.type === "comment" || n.type === "adjustments" || n.type === "rejected") {
                        localStorage.setItem("proposals_assembler_open_feedback", "true");
                    }
                    
                    router.push("/dashboard/crm/assembler/new/editor");
                } else {
                    router.push("/dashboard/crm/assembler");
                }
            } catch {
                router.push("/dashboard/crm/assembler");
            }
        } else if (n.dealId) {
            router.push(`/dashboard/crm/pipeline?dealId=${n.dealId}`);
        }
    };

                    ...
                    <div className="space-y-1">
                        ...
                        {(crmExpanded || collapsed) && (
                            <div className="space-y-1">
                                ...
                                {hasModuleAccess("crm") && (
                                    <Link href="/dashboard/whatsapp/kanban" className={`${navItemBase} ${pathname.includes('/whatsapp') && !pathname.includes('/whatsbot') ? 'bg-emerald-600/10 text-emerald-400 hover:bg-emerald-600/20' : navItemInactive}`}>
                                        <MessageCircle size={20} className={pathname.includes('/whatsapp') && !pathname.includes('/whatsbot') ? 'text-emerald-400' : 'text-slate-400'} />
                                        {!collapsed && <span>WhatsApp Web</span>}
                                    </Link>
                                )}

                                {hasModuleAccess("crm") && (
                                    <Link href="/dashboard/crm/whatsbot" className={`${navItemBase} ${pathname.includes('/whatsbot') ? 'bg-violet-600/10 text-violet-400 hover:bg-violet-600/20' : navItemInactive}`}>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={pathname.includes('/whatsbot') ? 'text-violet-400' : 'text-slate-400'}><path d="M12 8V4H8"/><rect width="16" height="12" x="4" y="8" rx="2"/><path d="M2 14h2"/><path d="M20 14h2"/><path d="M15 13v2"/><path d="M9 13v2"/></svg>
                                        {!collapsed && <span>Whatsbot</span>}
                                    </Link>
                                )}
                            </div>
                        )}

...
// These local functions are kept as empty ops to not break existing code 
// that might call them during transition, though they don't do anything securely anymore.
export function setTokens(accessToken: string, refreshToken: string) {
    // No-op: cookies are set automatically by the browser
    localStorage.setItem("accessToken", accessToken); // fallback logic can be removed later
    localStorage.setItem("refreshToken", refreshToken); // fallback logic can be removed later
}
    ...
    if (response.status === 401 && !skipAuth) {
        ...
        if (refreshToken) {
            const refreshResponse = await fetch(`${API_BASE_URL}/api/auth/refresh`, {
                ...
                credentials: "include", // Automatically uses refreshToken cookie
                body: JSON.stringify({ refreshToken }), // keep fallback body if needed
            });

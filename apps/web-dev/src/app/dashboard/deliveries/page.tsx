...
import {
    Package, Search, Building2, ChevronRight, Clock, CheckCircle2,
    AlertTriangle, Rocket, PlayCircle,
} from "lucide-react";
    ...
    return (
        <div className="p-6 md:p-8 space-y-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                ...
                <p className="text-slate-400 mt-1">Gerencie sprints e entregas de cada projeto.</p>
            </motion.div>
            ...
            {!loading && (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {filtered.map((project, i) => (
                        ...
                        >
                            <button onClick={() => router.push(`/dashboard/deliveries/${project.id}`)} className="w-full text-left">
                                <div className="group p-5 rounded-xl bg-slate-800/40 border border-white/[0.06] hover:border-purple-500/20 hover:shadow-xl hover:shadow-purple-500/5 transition-all">
                                    <div className="flex items-start justify-between mb-4">
                                        <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-purple-600/20 to-violet-500/10 border border-purple-600/20 flex items-center justify-center">
                                            <Package size={20} className="text-purple-400" />
                                        </div>
                                        <ChevronRight size={18} className="text-slate-600 group-hover:text-purple-400 group-hover:translate-x-0.5 transition-all" />
                                    </div>
